    ...
    , ui(new Ui::AuthenFriend),_label_point(2,6)
{
    ...
    ui->backEdit->setPlaceholderText("鸠魁");
    ...
    connect(ui->moreLabel, &ClickedOnceLabel::clicked, this, &AuthenFriend::ShowMoreLabel);
    InitTipLbs();
    //链接输入标签回车事件
    connect(ui->lbEdit, &CustomizeEdit::returnPressed, this, &AuthenFriend::SlotLabelEnter);
    connect(ui->lbEdit, &CustomizeEdit::textChanged, this, &AuthenFriend::SlotLabelTextChange);
    connect(ui->lbEdit, &CustomizeEdit::editingFinished, this, &AuthenFriend::SlotLabelEditFinished);
    connect(ui->tipLb, &ClickedOnceLabel::clicked, this, &AuthenFriend::SlotAddFirendLabelByClickTip);
    ...
    //连接确认和取消按钮的槽函数
    connect(ui->cancelBtn, &QPushButton::clicked, this, &AuthenFriend::SlotApplyCancel);
    connect(ui->sureBtn, &QPushButton::clicked, this, &AuthenFriend::SlotApplySure);
}
...
void AuthenFriend::SetApplyInfo(std::shared_ptr<ApplyInfo> apply_info)
{
    ...
    ui->backEdit->setPlaceholderText(apply_info->_name);
}
...
void AuthenFriend::resetLabels()
{
    ...
    if(_friend_labels.isEmpty()){
        ui->lbEdit->move(_label_point);
        return;
    }

    if(_label_point.x() + MIN_APPLY_LABEL_ED_LEN > ui->gridWidget->width()){
        ui->lbEdit->move(2,_label_point.y()+label_height+6);
    }else{
        ui->lbEdit->move(_label_point);
    }
}
...
void AuthenFriend::addLabel(QString name)
{
    ...
    if (_label_point.x() + MIN_APPLY_LABEL_ED_LEN > ui->gridWidget->width()) {
        ui->lbEdit->move(2, _label_point.y() + tmplabel->height() + 2);
    }
    else {
        ui->lbEdit->move(_label_point);
    }
...
void AuthenFriend::SlotLabelEnter()
{
    ...
    lb->setText(text);
    connect(lb, &ClickedLabel::clicked, this, &AuthenFriend::SlotChangeFriendLabelByTip);
    qDebug() << "ui->labelList->width() is " << ui->labelList->width();
