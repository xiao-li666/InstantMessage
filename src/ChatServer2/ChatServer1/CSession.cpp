...
int CSession::GetUseruid()
{
	return _useruid;
}
void CSession::SetUserUid(int uid)
{
	_useruid = uid;
}
void CSession::Start()
...
void CSession::AsyncReadHead(std::size_t length)
{
	...
	asyncReadFull(length, [self, this,length](const boost::system::error_code& err, std::size_t bytes_transfered) {
		try{
			...
			//判断链接无效
			if (!_server->checkValid(_sessionuid)) {
				Close();
				return;
			}
...
void CSession::Send(std::string msg, std::size_t msgid)
{
	...
	if (sendQueSize > MAX_SENDQUE) {
		std::cout << "session: " << _sessionuid << " send que fulled, size is " << MAX_SENDQUE << std::endl;
		return;
	}
