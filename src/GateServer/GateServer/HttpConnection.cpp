...
void HttpConnection::CheackDeadline()
{
	auto self = shared_from_this();
	deadline_.async_wait([self](boost::system::error_code ec) {
		if (!ec) {
			self->socket_.close(ec);//关闭连接
		}
		});
}
...
void HttpConnection::HandleRequest()
{
	...
	response_.keep_alive(false);//短链接
	if(request_.method() == http::verb::get){
		...
		if (!success) {
			...
			beast::ostream(response_.body()) << "The resource '" << request_.target() << "' was not found.";
